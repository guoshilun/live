...
import com.jk.jkproject.net.okhttp.HttpConfig;
import com.jk.jkproject.net.okhttp.RequestParams;
import com.jk.jkproject.net.okhttp.RequestTask;
import com.jk.jkproject.net.okhttp.ResponseListener;
import com.jk.jkproject.net.okhttp.SimpleResponseHandler;
import com.jk.jkproject.net.okhttp.base.BaseResponseHandler;
import com.jk.jkproject.net.okhttp.progress.down.DownTask;
import com.jk.jkproject.net.okhttp.progress.handler.ProgressHandler;
import com.jk.jkproject.net.okhttp.progress.listener.DownloadListener;
import com.jk.jkproject.net.okhttp.progress.listener.UploadListener;
import com.jk.jkproject.net.okhttp.progress.update.ProgressRequestBody;
...
//    }
    ...
    private Request.Builder createRequestBuilder(String tag, String fullUrl) {
        return new Request.Builder().url(fullUrl).tag(tag).headers(mHeadersBuilder.build());
    }
    ...
    private Request.Builder getPostBuilder(String url, RequestParams requestParams) {
        ...
        if (requestParams != null) {
            ...
            LogUtils.d("request params : " + requestParams.toString());
...
//            params = null;
    ...
    public void upload(String url, String fileKey, String filePath, RequestParams requestParams, UploadListener updateListener) {
        ...
        if (!file.exists()) {
            LogUtils.e("update file not exists");
            return;
        }
...
//        }
     ...
     * @param cls           the Json class which need to be parsed
     */
    public void postFeedLike(String url, RequestParams requestParams, ResponseListener listener, Class<?> cls) {
        String oid = "";
        if (requestParams != null) {
            oid = requestParams.get("oid");
        }

        Request request = getPostBuilder(url, requestParams).build();
        RequestTask task = new RequestTask(url, getOkHttpClient(), request, new SimpleResponseHandler(request.tag().toString(), listener), cls, oid);
        submit(task);
    }
