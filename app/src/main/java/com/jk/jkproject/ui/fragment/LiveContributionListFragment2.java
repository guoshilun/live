...
import com.jk.jkproject.ui.adapter.LiveContributionAdapter2;
 ...
 * @desc 榜单Fragment
 */
public class LiveContributionListFragment2 extends BFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private LiveContributionAdapter2 mAdapter;
    ...
    public void getData(int page) {
        ...
        switch (mType) {
            case 1:
                AppApis.getFanContributionList("", tabName, page, Urls.CURRENTCOUNT, this);
                break;
            case 2:
                AppApis.getHostContributionRankList(tabName, page, this);
                break;
        }
    }
...
//        getData(page);
    ...
    public static LiveContributionListFragment2 newInstance(int type, int position) {
        LiveContributionListFragment2 liveHomeNearFragment = new LiveContributionListFragment2();
    ...
    private void setData(FanContributionList info) {
        if (info == null || info.getData() == null || info.getData().size() == 0) {
            ...
            EventBus.getDefault().post(new PostResult(Constants.EVENTBUS.EVENTBUS_CONTRIBUTION_HOST_EMPLY, 1));
            return;
        }
        isHasData = info.getData().size() >= Integer.parseInt(Urls.CURRENTCOUNT) ? 1 : 0;
        EventBus.getDefault().post(new PostResult(Constants.EVENTBUS.EVENTBUS_CONTRIBUTION_HOST_EMPLY, 2));
        //当触发下拉刷新时
        if (isRefresh) {
            mSwipeRefresh.setRefreshing(false);
            isRefresh = false;
...
//                mSwipeRefresh.setLoading(false);
    ...
    private void setTop1(FanContributionList.DataBean dataBean) {
        ...
        tvHotCount1.setText(dataBean.getSum_hot() + "热度");
        tvUserLevel1.setText(dataBean.getAnchorGrade() + "");
        sdvCrown1.setOnClickListener(v -> setOnHead(dataBean.getUserId()));
        tvHostType1.setOnClickListener(v -> {
            position = 1;
            setHotType(dataBean);
        });
        UserLevelSetUtils.setHostLevel(tvUserLevel1, dataBean.getAnchorGrade());
        switch (dataBean.getR_state()) {
            case 1:
...
//                        tvHostType1.setBackgroundResource(R.drawable.bg_follow_red);
    ...
    private void setTop2(FanContributionList.DataBean dataBean) {
        ...
        tvHotCount2.setText(dataBean.getSum_hot() + "热度");
        tvUserLevel2.setText(dataBean.getAnchorGrade() + "");
        sdvCrown2.setOnClickListener(v -> setOnHead(dataBean.getUserId()));
        tvHostType2.setOnClickListener(v -> {
            position = 2;
            setHotType(dataBean);
        });
        UserLevelSetUtils.setHostLevel(tvUserLevel2, dataBean.getAnchorGrade());
        switch (dataBean.getR_state()) {
            case 1:
//                tvHostType1.setText("直播中");
...
//                        tvHostType1.setBackgroundResource(R.drawable.bg_follow_red);
    ...
    private void setTop3(FanContributionList.DataBean dataBean) {
        ...
        tvHotCount3.setText(dataBean.getSum_hot() + "热度");
        tvUserLevel3.setText(dataBean.getAnchorGrade() + "");
        sdvCrown3.setOnClickListener(v -> setOnHead(dataBean.getUserId()));
        tvHostType3.setOnClickListener(v -> {
            position = 3;
            setHotType(dataBean);
        });
        UserLevelSetUtils.setHostLevel(tvUserLevel3, dataBean.getAnchorGrade());
        switch (dataBean.getR_state()) {
            case 1:
//                tvHostType1.setText("直播中");
...
//                        tvHostType1.setBackgroundResource(R.drawable.bg_follow_red);
    ...
    protected void initData() {
        super.initData();
//        AppApis.getHostRank("", tabName, this);
        Drawable drawable = getResources().getDrawable(R.mipmap.live_icon_host_level);
        drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
        tvUserLevel1.setCompoundDrawables(drawable, null, null, null);
        tvUserLevel2.setCompoundDrawables(drawable, null, null, null);
        tvUserLevel3.setCompoundDrawables(drawable, null, null, null);
        backdrop.setImageResource(R.mipmap.icon_host_contribution_bg);
    }
    ...
    protected void initView(View paramView) {
        ...
        mAdapter = new LiveContributionAdapter2(getActivity(), list, 2);
        ...
        mAdapter.setOnItemClickListener(this::setOnClick);
    ...
    public void onSuccess(String url, Object obj) {
        ...
        if (Urls.GET_FAN_CONTRIBUTION_LIST.equals(url) || Urls.GET_HOST_RANK_LIST.equals(url)) {
