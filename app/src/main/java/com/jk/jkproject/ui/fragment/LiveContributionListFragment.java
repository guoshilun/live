 ...
 * @desc 榜单Fragment
 */
public class LiveContributionListFragment extends BFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public static LiveContributionListFragment newInstance(int type, int position) {
        LiveContributionListFragment liveHomeNearFragment = new LiveContributionListFragment();
    ...
    private void setData(FanContributionList info) {
        if (info == null || info.getData() == null || info.getData().size() == 0) {
            ...
            EventBus.getDefault().post(new PostResult(Constants.EVENTBUS.EVENTBUS_CONTRIBUTION_USER_EMPLY, 1));
            return;
        }
        isHasData = info.getData().size() >= Integer.parseInt(Urls.CURRENTCOUNT) ? 1 : 0;
        EventBus.getDefault().post(new PostResult(Constants.EVENTBUS.EVENTBUS_CONTRIBUTION_USER_EMPLY, 2));
        //当触发下拉刷新时
        if (isRefresh) {
            mSwipeRefresh.setRefreshing(false);
            isRefresh = false;
...
//                mSwipeRefresh.setLoading(false);
    ...
    private void setTop1(FanContributionList.DataBean dataBean) {
        ...
        tvHotCount1.setText(dataBean.getSum_amount() + "钻石");
...
//                        tvHostType1.setBackgroundResource(R.drawable.bg_follow_red);
    ...
    private void setTop2(FanContributionList.DataBean dataBean) {
        ...
        tvHotCount2.setText(dataBean.getSum_amount() + "钻石");
    ...
    private void setTop3(FanContributionList.DataBean dataBean) {
        ...
        tvHotCount3.setText(dataBean.getSum_amount() + "钻石");
    ...
    protected void initData() {
        super.initData();
        backdrop.setImageResource(R.mipmap.icon_use_contribution_bg);
    }
