package com.miliyo.danmaku.model.collection;
...
import com.miliyo.danmaku.model.painter.L2RPainter;
import com.miliyo.danmaku.model.painter.R2LPainter;
...
//        }
    ...
    private synchronized void drawEveryElement(ArrayList<DanMuModel> danMuViewQueue, Canvas canvas) {
        ...
        if (danMuChannelInfo == null) {
            danMuChannelInfo = new DanMuChannelInfo();
        }
        for (int i = 0; i < (danMuViewQueue.size() > MAX_COUNT_IN_SCREEN ? MAX_COUNT_IN_SCREEN : danMuViewQueue.size()); i++) {
            ...
            if (danMuView.isAlive()) {
                ...
                danMuChannelInfo.danmuSize = danMuViewQueue.size();
                danMuChannelInfo.lastChannel = danMuViewQueue.get(danMuViewQueue.size() - 1).getChannelIndex();
                if (danMuView.isAttached()) {
                    performDraw(danMuView, danMuPainter, canvas, danMuChannel);
                }
            } else {

                if (danMuViewQueue.size() == 1) {
                    danMuChannelInfo.danmuSize = 0;
                    danMuChannelInfo.lastChannel = -1;
                }
...
//            danMuChannel.space = selectSpaceRandomly();
    ...
    private DanMuChannelInfo danMuChannelInfo = new DanMuChannelInfo();

    public DanMuChannelInfo getDanMuSizeInScreen() {
        if (danMuChannelInfo == null) {
            danMuChannelInfo = new DanMuChannelInfo();
        }
        return danMuChannelInfo;
    }
}
